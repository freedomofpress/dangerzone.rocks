...
Dangerzone uses Linux containers, which are isolated application environments that share the Linux kernel with their host. On Windows and macOS, it users [Podman](https://podman.io/) under the hood, which allows Dangerzone to spin containers in a dedicated virtual machine. Since Dangerzone 0.10.0, all this complexity is hidden from the user.
